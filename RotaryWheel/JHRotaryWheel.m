...
- (id)initWithFrame:(CGRect)frame andDelegate:(id)delegate withLabels:(NSArray *)labels{
    if ((self = [super initWithFrame:frame])) {
        UIImageView *background = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"JH_RotaryWheel_Background"]];
...
-(void)drawWheel {
    ...
    for (int i = 0; i < _numberOfSections; i++) {
        ...
        if (i == 0) {
            im.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"JH_RotaryWheel_BottomArrow.png"]];
        } else if (i == _numberOfSections / 2.f) {
            im.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"JH_RotaryWheel_TopArrow.png"]];
        } else {
