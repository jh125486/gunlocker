...
- (IBAction)saveTapped:(id)sender {
    ...
    newDopeCard.notes = [NSString stringWithFormat:@"Generated on %@", [NSDate date]];// set DopeCard.notes to trajectory autogenerated Date
    newDopeCard.range_unit = self.rangeUnit;
    newDopeCard.drop_unit = newDopeCard.drift_unit = self.dropDriftUnit;

    newDopeCard.dope_data = [[NSMutableArray alloc] initWithCapacity:trajectory.ranges.count];
    for (TrajectoryRange *range in trajectory.ranges) {
        [newDopeCard.dope_data addObject:range.range_yards];
        [newDopeCard.dope_data addObject:range.drop_inches];
        [newDopeCard.dope_data addObject:range.drift_inches];
    }
